  ...
  return (
    <section>
      <table>
        ...
        <tbody>
          {rows.routes.slice(pageStart, pageStart + perPage).map(info => {
            return (
      ...
      <p>Showing {(pageStart) + 1}-{(page * perPage) + perPage} of {rows.routes.length} routes.</p>
      <button disabled={page === 0} onClick={previousPage}>previous</button>
      <button disabled={pageStart + perPage >= rows.routes.length} onClick={nextPage}>next</button>
    </section>
  )
}
